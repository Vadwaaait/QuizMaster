...
const loadQuestion = () => {
    ...
    //const questiondata = `data.${para}[${index}]`;
    const questiondata = data.modernart[index];

    console.log(questiondata.question)

    quebox.innerText = `${index+1}) ${questiondata.question}`;
    optionInputs[0].nextElementSibling.innerText = questiondata.options[0];
    optionInputs[1].nextElementSibling.innerText = questiondata.options[1];
    optionInputs[2].nextElementSibling.innerText = questiondata.options[2];
    optionInputs[3].nextElementSibling.innerText = questiondata.options[3];
}
...
const submitQuiz = () => {
    let rans = data.modernart[index].answer;
    const ans = getAns();

    console.log(ans, rans);
    if (ans == rans) {
        rightAns++;
    } else {
        wrongAns++;
...
const getAns = () => {
    ...
    optionInputs.forEach(
        (input) => {
            if (input.checked) {
                uanswer = input.nextElementSibling.innerText;
...
const endQuiz = () => {
    const percentage = rightAns / total * 100;


    document.getElementById("box").innerHTML = `
    <h1 class="subsuccess"> The Modern-Art Quiz has been Submitted </h3>

    
        <h2>Result = ${rightAns} / ${total}  </h2>
        <h2>Right Answers=  ${rightAns} </h2>
        <h2>Wrong Answers=${wrongAns} </h2>
        <h2>Pergentage = ${percentage}%</h2>
